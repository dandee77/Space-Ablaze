...
void Button::draw()
{
    ...
    case BUTTON_LEFT_SIDE:
        ...
        DrawTextEx(GetFontDefault(), m_text.c_str(), (Vector2){ m_rect.x + 10, m_rect.y }, m_size, 2.0, ((isHovered()) ? BLACK : m_color));
        break;

    case BUTTON_CENTERED:

        Vector2 text_dimensions = MeasureTextEx(GetFontDefault(), m_text.c_str(), m_size, 0);
        float text_x = m_rect.x + (m_rect.width - text_dimensions.x) / 2;
        float text_y = m_rect.y + (m_rect.height - text_dimensions.y) / 2;
        DrawTextEx(GetFontDefault(), m_text.c_str(), (Vector2){ text_x, text_y }, m_size, 2.0, ((isHovered()) ? BLACK : m_color));
        break;
    }
}